...
const TeamDetails = props => (
    ...
    <header>
      <img className="team-photo" src={props.team.imageUrl} alt={props.team.name}/>
    </header>{" "}
    <div className="team-info">
      <h1> {props.team.name} </h1> <h2> {props.team.conference} </h2>{" "}
      <h2> {props.team.division} </h2>
      <div className="info">
        <div>
          <span> Hala: </span> <span> {props.team.arena} </span>{" "}
        </div>
        <div>
          <span> Trener: </span> <span> {props.team.head_coach} </span>{" "}
        </div>
        <div>
